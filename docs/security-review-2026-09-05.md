...
The reviewed scope was published as experimental source on 2026-09-05 after the required checks
passed and the repository owner accepted the historical metadata disclosure. It is not safe for
automatic execution of code produced by an issue, recorder, model or untrusted contributor. The current alpha requires a
...
| PUB-01 | Publication privacy | Existing Git history contains the owner's Outlook commit email and a superseded D-drive experiment path. | No credential signature was found. Locally authored task commits use GitHub noreply; GitHub's hosted merges use the account's allowed default email. On 2026-09-05 the owner accepted the disclosure and explicitly authorized public visibility. Publication completed without rewriting evidence-bound history. |
...
identity. GitHub's PR #10/#11 merge commits also retained the account's default Outlook author email.
Those immutable merges were included in the disclosure accepted before publication. This was not
a full entropy scanner: gitleaks and trufflehog were not installed.
...
in that remote surface. The final pre-publication receipt at `2026-09-05T07:07:06.373Z` extended
coverage to all 11 PRs and 27 completed workflow logs, with no pending run, downloadable artifact
or match for the same credential/known-owner signatures. Its SHA-256 is
...
was not exposed then. After owner-authorized publication on 2026-09-05, private vulnerability
reporting, secret scanning and push protection were enabled and read back successfully. A fresh
read during publication-record recovery confirmed these settings and found zero open
secret-scanning alerts. This verifies configuration and a point-in-time alert result; it does not
claim that background scanning is complete or that arbitrary private data is absent.
